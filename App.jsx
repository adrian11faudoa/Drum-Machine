...
class DrumPad extends React.Component {
  ...
  activatePad() {
    if (this.props.power) {
      this.setState(prev =>
        prev.padStyle.backgroundColor === "orange" ? { padStyle: inactiveStyle } : { padStyle: activeStyle }
      );
    } else {
      this.setState({
        padStyle: {
...
class PadBank extends React.Component {
  render() {
    const pads = this.props.currentPadBank.map((pad, i) => (
      <DrumPad
        clip={this.props.power ? pad.url : "#"}
        clipId={pad.id}
        key={pad.id}
        keyCode={pad.keyCode}
...
class App extends React.Component {
  ...
  render() {
    ...
    // update volume
    Array.from(document.getElementsByClassName("clip")).forEach(sound => {
